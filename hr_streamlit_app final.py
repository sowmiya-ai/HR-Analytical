...
# HR Analytics Dashboard - Google Web App converted to Streamlit
...
def show_dashboard(filtered: pd.DataFrame, all_rows: pd.DataFrame) -> None:
    ...
    c1.plotly_chart(px.bar(dept_counts, x="Department", y="Employee Count", text="Employee Count"), use_container_width=True)
    ...
    c3.plotly_chart(px.bar(age_band_counts(filtered), y="Age Band", x="Employee Count", orientation="h", text="Employee Count"), use_container_width=True)
